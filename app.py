...
DATA_URL = "MVC_-_Crashes.csv"

st.title("Vehicle Collisions in New York City")
st.markdown("This application is a dashboard that can be used to analyze motor vehicle collision in New York city 🚗")


@st.cache(persist=True)  # help to not do the samething on everytime the app loads
def load_data(nrows):
...
st.header("Breakdown by minutes between %i:00 and %i:00" % (hour, (hour + 1) % 24))
filtered = data[
...
if select == 'Pedestrians':
    ...
    st.write(original_data.query('injured_xyclists >= 1')[['on_street_name', 'injured_xyclists']].sort_values(
